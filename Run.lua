...
dezzer = Token:match("(%d+)")
redis:set(dezzer..":VERSION",1)
redis:set(dezzer..":SUDO_ID:",SUDO_USER)
redis:set(dezzer..":DataCenter:",'German')
redis:set(dezzer..":UserNameBot:",BOT_User)
redis:set(dezzer..":NameBot:",BOT_NAME)
redis:hset(dezzer..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3dezzer_INSTALL","Yes")
info = {} 
...
dezzer = Token:match("(%d+)")
our_id = tonumber(dezzer)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(dezzer..":UserNameBot:")
SUDO_ID = tonumber(redis:get(dezzer..":SUDO_ID:"))
SUDO_USER = redis:hgetall(dezzer..'username:'..SUDO_ID).username
version = redis:get(dezzer..":VERSION")
DataCenter = redis:get(dezzer..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..dezzer..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(dezzer..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(dezzer..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(dezzer..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(dezzer..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور '
msg.TheRank = 'مطور البوت '
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(dezzer..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي '
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(dezzer..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء '
msg.TheRank = 'المنشىء '
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(dezzer..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير ' 
msg.TheRank = 'مدير البوت ' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(dezzer..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن '
msg.TheRank = 'ادمن في البوت '
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(dezzer..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(dezzer..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(dezzer..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(dezzer..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(dezzer..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(dezzer..':WELCOME_BOT'),[[•~مـرحبآ آنآ بوت آسـمـي ]]..redis:get(dezzer..':NameBot:')..[[•~آختصـآصـي حمـآيه‌‏ آلمـجمـوعآت
...
if msg.GroupActive and redis:get(dezzer..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(dezzer..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(dezzer..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(dezzer..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.dezzer then 
if msg.text and ScriptFile.idezzer then
for k, dezzer in pairs(ScriptFile.dezzer) do
local SearchText = msg.text:match(dezzer)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.idezzer(msg,{msg.text:match(dezzer)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",dezzer.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.ddezzer then
if ScriptFile.ddezzer(msg) == false then
return false
end
...
if Plug.dezzer then 
if msg.text and Plug.idezzer then
for k, dezzer in pairs(Plug.dezzer) do
local SearchText = msg.text:match(dezzer)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(dezzer..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(dezzer..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.idezzer(msg,{msg.text:match(dezzer)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",dezzer.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
if Plug.ddezzer then
Plug.ddezzer(msg)
print("\27[1;35m¦Msg_IN_Process : \27[1;32"..name.."\27[0m")
end
else
print("The File "..name.." Not Runing in The Source dezzer")
end 

end
else
print("The File Script.lua Not Runing in The Source dezzer")
...
function tdcli_update_callback(data)
	...
	redis:srem(dezzer..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/devsonicx/dezzer/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/devsonicx/dezzer/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/devsonicx/dezzer/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/devsonicx/dezzer/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/devsonicx/dezzer/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/devsonicx/dezzer/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(dezzer..":SUDO_ID:")
redis:hset(dezzer..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(dezzer..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(dezzer..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(dezzer..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(dezzer..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(dezzer..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(dezzer..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(dezzer..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(dezzer..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(dezzer..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(dezzer..'group:add'..data.chat_.id_) then
	redis:set(dezzer..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(dezzer..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(dezzer..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(dezzer..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	dezzer..'group:name'..k,IDS.Title,
	dezzer..'num_msg_dezzer'..k,5,
	dezzer..'group:add'..k,true,
	dezzer..'lock_link'..k,true,
	dezzer..'lock_spam'..k,true,
	dezzer..'lock_webpage'..k,true,
	dezzer..'lock_markdown'..k,true,
	dezzer..'lock_flood'..k,true,
	dezzer..'lock_bots'..k,true,
	dezzer..'mute_forward'..k,true,
	dezzer..'mute_contact'..k,true,
	dezzer..'mute_document'..k,true,
	dezzer..'mute_inline'..k,true,
	dezzer..'lock_username'..k,true,
	dezzer..'replay'..k,true
	)
	redis:sadd(dezzer..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(dezzer..'username:'..ID,'username',user)
	redis:sadd(dezzer..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(dezzer..'username:'..ID,'username',user)
	redis:sadd(dezzer..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(dezzer..'username:'..ID,'username',user)
	redis:sadd(dezzer..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'•~** تم رفع آلنسـخه‏‏ آلآحتيآطـيه\n•~** حآليآ عدد مـجمـوعآتك هيه *'..redis:scard(dezzer..'group:ids')..'* ®️\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(dezzer..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(dezzer..'username:'..data.user_.id_).username
	...
	redis:hset(dezzer..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
